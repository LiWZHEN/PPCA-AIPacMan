...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    combinations = [c for c in itertools.combinations(literals, 2)]
    target = []
    for combination in combinations:
        possibility = disjoin([~combination[0], ~combination[1]])
        target.append(possibility)
    return conjoin(target)
    # "*** END YOUR CODE HERE ***"
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        KB.append(exactlyOne([PropSymbolExpr(pacman_str, x, y, time = t) for x, y in non_wall_coords]))
        is_goal = conjoin([~PropSymbolExpr(food_str, x, y, time = t) for x, y in food])
        model = findModel(conjoin(KB + [is_goal]))
        if model is not False:
            return extractActionSequence(model, actions)
        KB.append(exactlyOne([PropSymbolExpr(action, time = t) for action in actions]))
        for x, y in non_wall_coords:
            successor_axioms = pacmanSuccessorAxiomSingle(x, y, t + 1, walls)
            if successor_axioms is not None:
                KB.append(successor_axioms)
        for x, y in food:
            KB.append(PropSymbolExpr(food_str, x, y, time = t + 1) % conjoin([PropSymbolExpr(food_str, x, y, time = t), ~PropSymbolExpr(pacman_str, x, y, time = t)]))
    return []
    # "*** END YOUR CODE HERE ***"
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for x, y in non_outer_wall_coords:
            pacman_xy_t = PropSymbolExpr(pacman_str, x, y, time = t)
            pacman_at_xy = entails(conjoin(KB), pacman_xy_t)
            pacman_not_at_xy = entails(conjoin(KB), ~pacman_xy_t)
            if pacman_not_at_xy:
                KB.append(~pacman_xy_t)
            else:
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for x, y in non_outer_wall_coords:
            ...
            has_wall_at_xy = entails(conjoin(KB), wall_xy)
            not_has_wall_at_xy = entails(conjoin(KB), ~wall_xy)
            if has_wall_at_xy:
                known_map[x][y] = 1
                KB.append(wall_xy)
            if not_has_wall_at_xy:
                known_map[x][y] = 0
                KB.append(~wall_xy)
        agent.moveToNextState(action_t)
        "*** END YOUR CODE HERE ***"
