...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict[OBS0] = [d for d in range(X_RANGE + Y_RANGE + MAX_NOISE - 1)]
    variableDomainsDict[OBS1] = [d for d in range(X_RANGE + Y_RANGE + MAX_NOISE - 1)]
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for variable_to_eliminate in eliminationOrder:
            join_factors = []
            remain_factors = []
            for factor in factors:
                if variable_to_eliminate in factor.variablesSet():
                    join_factors.append(factor)
                else:
                    remain_factors.append(factor)
            if join_factors:
                current_not_join, joined_factor = joinFactorsByVariable(join_factors, variable_to_eliminate)
                if len(joined_factor.unconditionedVariables()) > 1:
                    eliminated_joined_factor = eliminate(joined_factor, variable_to_eliminate)
                    remain_factors.append(eliminated_joined_factor)
            factors = remain_factors
        joined_factor = joinFactors(factors)
        return normalize(joined_factor)
        # "*** END YOUR CODE HERE ***"
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        if total_sum != 0:
            ...
            random_value *= total_sum
            add_up = 0
            for key, value in self.items():
                add_up += value
                if add_up > random_value:
                    return key
        return None
        # "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        particle_num_per_pos = self.numParticles // legal_pos_num
        for legalPosition in self.legalPositions:
            self.particles += [legalPosition] * particle_num_per_pos
        remained_particle_num = self.numParticles - particle_num_per_pos * legal_pos_num
        for i in range(remained_particle_num):
